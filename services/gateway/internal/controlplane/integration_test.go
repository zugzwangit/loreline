...
func TestProductionLifecycle(t *testing.T) {
	...
	if python := os.Getenv("LORELINE_TEST_PYTHON"); python != "" {
		...
		cmd.Env = append(os.Environ(), "LORELINE_DATABASE_URL="+url, "LORELINE_WORKER_POLL_SECONDS=0.05")
	...
	decision := call("POST", "/v1/candidates/"+candidate+"/decision", `{"action":"approve","note":"verified"}`)
