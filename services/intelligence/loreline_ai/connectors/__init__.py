...
def build_connector(config:dict[str,Any], cfg:Settings|None=None)->Connector:
    ...
    if provider=="http_json":return HTTPJSONConnector(config,runtime)
    if provider=="zendesk":return ZendeskConnector(config,runtime)
    if provider=="confluence":return ConfluenceConnector(config,runtime)
