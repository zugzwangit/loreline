...
class HTTPJSONConnector:
    ...
    def fetch(self,cursor:dict[str,Any])->tuple[list[SourceRecord],dict[str,Any]]:
        ...
        response=httpx.get(self.url,headers=headers,params=params,timeout=30,follow_redirects=False);response.raise_for_status();data=response.json();raw=nested(data,self.records_path,[])
        if not isinstance(raw,list):raise ValueError("connector records_path did not resolve to a list")
        records=[SourceRecord(str(nested(item,self.id_path)),str(nested(item,self.title_path,"Untitled")),str(nested(item,self.content_path)),metadata={"connector":"http_json"}) for item in raw]
        next_value=nested(data,self.cursor_path,"");return records,{"value":next_value} if next_value else cursor
